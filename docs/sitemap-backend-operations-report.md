...
> **목표:** 12개월 SEO 전략 보고서를 *지속 가능하게 운영*하기 위한 사이트맵과 백엔드 데이터 모델을 설계한다.
...
이 보고서의 모든 논의는 다음 한 문장에서 출발합니다.

> **사이트맵을 *그리는 것*이 아니라 *데이터 모델로부터 결정론적으로 도출되는 것*으로 설계한다.**

이게 왜 중요한가? 사이트맵을 *손으로 그리면* 6개월 안에 다음이 모두 일어납니다.
...
| 새 지역이 추가됐는데 어디 둘지 모름 | 구조의 결정 규칙이 명문화 안 됨 |
| 약한 페이지가 누적되어 도메인 권위 하락 | 정리 트리거가 자동화 안 됨 |
...
**제약:** `(parent_id, slug)` UNIQUE. 같은 부모 아래에 동일 slug 금지.

#### AUTHOR (작성자)

E-E-A-T 신호의 원천. *반드시 실명* 기반.
...
| `verified_at` | TIMESTAMP | 신원 확인 일자 |
...
|------|------|------|
| `id` | INT, PK | |
| `theme_id` | INT, FK→THEME | 어떤 테마인가 |
| `region_id` | INT, FK→REGION | 어떤 지역인가 (NULL=지역 무관 허브) |
...
**제약 (가장 중요):**
- `(theme_id, region_id)` UNIQUE — *같은 테마-지역 쌍에 대해 노드는 정확히 하나*. 카니발리제이션 원천 차단.
- `status='live'`는 `data_count >= 5`일 때만 가능 (DB 트리거로 강제).

#### DATA_POINT (고유 데이터)

각 노드에 붙는 *고유 데이터의 단위*. 이게 페이지를 페이지답게 만드는 재료.
...
| `value` | JSONB | 실제 값 (스키마는 kind에 따라 다름) |
| `source` | VARCHAR | 출처 |
...
```
/                                            ← 사이트 허브 (theme=NULL, region=NULL은 특수 처리)
...
1. **카니발리제이션 자동 방지** — `(theme, region)` UNIQUE 제약으로 같은 페이지가 두 URL로 생성되는 일이 *DB 레벨에서* 불가능.
2. **계층 권위 흐름** — URL 깊이 = 권위 단계. 부모 URL이 자식 URL의 prefix.
3. **사이트맵 자동 생성** — `status='live'`인 노드를 SELECT하면 그게 sitemap.xml.
4. **운영자 인지 부하 최소화** — URL만 봐도 어느 데이터 행에 해당하는지 안다.
...
| 금지 | 이유 |
|------|------|
| `?theme=...&region=...` 쿼리스트링 URL | Google이 다른 페이지로 인식 안 함 |
...
**용도:** 매일 우선순위 결정. *◐ → ●*가 가장 효율적인 작업 (검색 의도는 검증됨, 데이터만 부족).
...
```
              인덱스   노출    클릭    평균순위   판단
/tutoring/      ✓     8,200    320     11.2    ✓ 정상
/.../seoul/     ✓     3,400    145     14.8    ✓ 정상
/.../gangnam/   ✓     1,200     62     18.3    ✓ 정상
/.../daechi/    ✗        15      0     62.1    ✗ 6개월 인덱스 안 됨 → 통합 검토
/.../yeoksam/   ✓       380     23     24.5    ◐ 보강 필요
/academy/       ✓     2,100     89     16.7    ✓ 정상
```

**용도:** 분기 정리 회의. 살릴지 / 통합할지 / 삭제할지 결정.
...
| 갱신 큐 처리 | 큐의 노드들 점검 | `last_review_at` 갱신 |
...
매일 새벽 노출·클릭·순위·CTR을 가져와 `kpi_snapshot`에 저장. KPI 히트맵과 갱신 큐의 데이터 소스.
...
- 끊긴 내부 링크
- 부모 자식 목록에서 누락된 자식
- depth가 부모와 정합되지 않는 노드

### 5.6 자동 리다이렉트 관리

```
노드 status를 'dead'로 바꾸면 자동으로:
  1. 가장 가까운 상위 live 노드 찾기
  2. 301 리다이렉트 등록
...
## 6. 데이터 모델의 자기 강제 — 무결성 트리거

사람이 잊거나 우회하지 못하도록 DB 레벨에서 강제하는 규칙.
...
본 보고서는 특정 스택을 강제하지 않지만, 다음 결정은 *전체 운영 방식에 직결*되므로 신중히 합니다.
...
| SSR (Server-Side Rendering) | ★★★☆☆ | DB 부하 + 응답 속도 부담. 캐싱 잘 안 하면 Core Web Vitals 위험. |
...
**권장: Phase 0~1은 로우코드 (Retool/Appsmith) → Phase 3 이후 자체 구축 검토.** 처음부터 자체 구축하면 SEO 작업이 늦어집니다.

---

## 8. 구현 로드맵 — 12개월 SEO 전략과의 동기화

선행 보고서의 Phase 구조와 동기화한 백엔드 구현 일정.
...
**대응:**
- 가능한 한 외부 데이터 소스에서 자동 수집 (학교 정보, 행정 데이터 등 공개 API)
- DataPoint 입력 UI를 *5초 안에 1건 추가 가능*하게 설계
- 주간 입력 목표 KPI 설정 (예: 주 50건)

### 9.2 발행 게이트 우회 시도

Month 2~3 충동 시기에 *"데이터 4개로 발행하면 안 될까?"* 가 반드시 옵니다.

**대응:**
- DB 트리거로 강제 (코드 외부 우회 불가)
- 임시 우회 기능을 *애초에 만들지 않습니다.* 비상 우회 버튼은 곧 일상 우회 버튼이 됩니다.

### 9.3 데이터-페이지 동기화 지연

DataPoint가 추가됐는데 페이지가 갱신 안 되면, *Google이 보는 페이지와 실제 데이터가 어긋납니다.*

**대응:**
- ISR 사용 시 `revalidate` 시간을 24시간 이하로
- DataPoint INSERT 시 해당 노드의 `last_review_at` 자동 갱신
- 매일 sitemap.xml에 lastmod 갱신

### 9.4 운영자 이직/교체

12개월 전략은 한 사람이 끝까지 끌고 가지 않을 수 있습니다.
...
3. **사이트맵은 5개 뷰**(발행 트리, 데이터 매트릭스, KPI 히트맵, 사일로 무결성, 갱신 큐)로 운영됩니다. 각 뷰는 다른 결정에 쓰입니다.

4. **6개 백엔드 기능**(발행 게이트, sitemap 생성기, SC 동기화, 카니발리제이션 검출, 사일로 검사, 자동 리다이렉트)이 사이트맵을 살아 있게 유지합니다.

5. **DB 트리거가 사람의 의지를 대체합니다.** Month 2~3 충동에 굴복하지 않을 시스템적 장치가 코드 레벨에서 강제됩니다.
...
다음이 정해져야 Phase 0 데이터 모델 구현에 진입할 수 있습니다.
...
2. **운영자 인터페이스를 어디까지 자체 구축할 것인가?**
   ...
   - 옵션 A를 추천합니다. SEO 작업이 늦어지면 12개월 계획이 지연됩니다.

3. **데이터 자동 수집 범위를 어디까지 잡을 것인가?**
   - 학교 정보, 행정 데이터: 공공 API로 자동 수집 가능
   - 매칭 가능 선생님, 사례, 후기: 운영 데이터에서 자동 추출 가능
...
- [ ] 어제 추가된 DataPoint 검증 (`verified = true`)
- [ ] 매트릭스 뷰에서 `◐` 셀 1개 이상을 `●`로 승급 시도
